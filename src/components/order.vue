...
import china from '../api/address3.json'
	export default {
		data() {
			return {
				detailInfo: false,
		...
		methods: {
			...
			onValuesChange(picker, values) {
				if(!values[0]) values[0] = '北京市'
		      	this.cities = Object.keys(china[values[0]]);
		      	picker.setSlotValues(1, this.cities);
		      	this.areas = china[values[0]][values[1]];
